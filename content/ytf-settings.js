...
(function () {
  ...
  browser.runtime.onMessage.addListener((msg) => {
    if (!msg) return;
    if (msg.type === "ytf-settings-update" && msg.settings) {
      Object.assign(settings, msg.settings);
      YTF.log("Settings updated:", JSON.stringify(msg.settings));
      YTF.resetAndRescan && YTF.resetAndRescan();
    } else if (msg.type === "ytf-test-simulate-failure") {
      YTF.simulateHealthFailure && YTF.simulateHealthFailure();
    } else if (msg.type === "ytf-run-health-now") {
      YTF.runHealthCheckNow && YTF.runHealthCheckNow();
    }
  });
